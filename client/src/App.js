...
function App() {
  ...
  const submitReview = () => {
    Axios.post('http://127.0.0.1:9000/crude', {
      name: sname,
      tech: tech,
      sub: status
    }).then(() => {
  ...
  const retrieveData = () => {
    Axios.get('http://127.0.0.1:9000/crude')
      .then((response) => {
  ...
  const updateData = () => {
    Axios.patch(`http://127.0.0.1:9000/crude/${id}`, {
      sub: status
    }).then(() => {
  ...
  const deleteData = () => {
    Axios.delete(`http://127.0.0.1:9000/crude/${id}`)
      .then(() => {
